...
class Gaussian():
    ...
    def Interferometer(self, U):
        ...
        BSang = [0]*(int(n*(n-1)/2))
        rot = [0]*(int(n*(n-1)/2))
        ### Introducing counter is OK. It's needed for even iteration of "i".
        count_odd = 0
        count_even = 0
        for i in range (1, n):
            if ((i+2)%2==1):
                for j in range (i): 
                    ...
                    BSang[count_odd+j] = alpha
                    rot[count_odd+j] = theta
                ...
                for j in range (i):
                    ...
                    BSang[-(count_even +1+j)] = alpha
                    rot[-(count_even +1+j)] = theta
                    e = np.cos(theta) + np.sin(theta)*1j
        ...
        params = np.angle(np.diag(U))
        for i in range(n):
            self.ops.append(GATE_SET['R'])
            self.ops[-1] = self.ops[-1](self, i, params[i])
